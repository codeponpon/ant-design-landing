...
export default class EditButtonView extends React.PureComponent {
  ...
  getMenu = (parent) => {
    const children = parent.map((item) => (
      <Menu.Item key={item.dataId}>{this.getStr(item.dataId)}</Menu.Item>
    ));
    return <Menu onClick={this.onParentDropdonw}>{children}</Menu>;
  };
  ...
  render() {
    ...
    if (parent && parent.length) {
      parentChild = (
        <Dropdown key="d" overlay={this.getMenu(parent)} placement="bottomRight">
      ...
      editButtonArray
        .map((key) => {
          switch (key) {
            ...
            case 'Menu':
            case 'LinkMenu':
              return <MenuComp {...this.props} key={key} />;
            case 'icon':
              return <IconComp {...this.props} key={key} />;
            case 'video':
              return <VideoComp {...this.props} key={key} />;
            case 'texty':
              return <TextyComp {...this.props} key={key} />;
            case 'Content': // Content 必须为 object;
              return <ContentWrapper {...this.props} key={key} />;
            case 'link':
            case 'linkA':
