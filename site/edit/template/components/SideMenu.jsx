...
class SideMenu extends React.PureComponent {
    ...
    const pushData = (child, i, key, item) => {
      ...
      children.push(
        ...
        >
          ...
          <p>
            {item.name}
            {child.uid} - {child[`text${isThTH ? '' : 'En'}`]}
          </p>
        </div>,
      );
    };
    Object.keys(webData)
      .sort((a, b) => webData[a].order - webData[b].order)
      .forEach((key) => {
        if (key !== 'Other' && !webData[key]?.disabled) {
  ...
  onSignUp = (value) => {
    ...
    message.success(this.props.intl.formatMessage({ id: 'app.side.encryption.message' }));
  };
  ...
  oonUnLockData = () => {
    ...
    message.success(this.props.intl.formatMessage({ id: 'app.side.decrypt.message' }));
  };
  ...
  handleLangChange = () => {
    ...
    const isThTH = utils.isThTH(pathname);
    if (utils.isLocalStorageNameSupported()) {
      localStorage.setItem('locale', isThTH ? 'en-US' : 'th-TH');
    }
    ...
    window.location.href =
      ...
      currentHref.replace(
        ...
        utils.getLocalizedPathname(
          isThTH ? window.location.pathname : `${window.location.pathname}index`,
          !isThTH,
        ),
      );
  };
  ...
  render() {
    ...
    return (
      <div className="edit-side-menu-wrapper" onMouseLeave={this.hideMenu}>
        <DrawerMenu
          level={null}
          getContainer={null}
          handler={null}
        ...
        >
          <div className="img-content-wrapper">{drawerChild}</div>
        </DrawerMenu>
        <div className="edit-side-menu">
          <div className={isThTH ? 'add add-zh' : 'add'} onMouseEnter={this.showMenu}>
            <PlusCircleOutlined />
          ...
          <ul className="other" onMouseEnter={this.hideMenu}>
            ...
            <Tooltip title={<FormattedMessage id="app.side.help" />} placement="right">
              <li>
                <a
                  href={`${getNewHref('7111', '', true, '/docs/edit/edit-block')}`}
                  target="_blank"
                >
                  <ExclamationCircleOutlined />
                </a>
